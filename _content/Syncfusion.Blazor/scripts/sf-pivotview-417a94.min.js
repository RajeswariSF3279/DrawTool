...
window.sfBlazor.PivotView = function () {
  ...
  var PIVOT_FIELD_LIST_CONTAINER_CLASS = 'e-pivotfieldlist-container';
  ...
  var CANCEL_BTN_CLASS = 'e-cancel-btn';
  var CLOSE_ICON = 'e-dlg-closeicon-btn.e-icon-btn';
  /**
  ...
  function () {
    ...
    PivotButton.prototype.getButtonPosition = function (target, droppedClass) {
      ...
      if (buttonElement) {
        ...
        var groupingBarLayout = this.parent instanceof SfPivotView ? this.parent.parentElement.querySelector('.' + CHART_GROUPING_BAR_CLASS) : undefined;
        var pivotButtons = [].slice.call((groupingBarLayout && this.parent.currentView === 'Chart' ? groupingBarLayout : this.parent.parentElement).querySelector('.e-' + droppedClass).querySelectorAll('.' + PIVOT_BUTTON_CLASS));
    ...
    CommonKeyboardInteraction.prototype.processClose = function (e) {
      ...
      if (target && sf.base.closest(target, '.' + POPUP + '.' + POPUP_OPEN)) {
        ...
        if (dialogInstance && !dialogInstance.closeOnEscape) {
          ...
          this.closeDialog(sf.base.closest(target, '.' + POPUP + '.' + POPUP_OPEN), e);
        }
      }
    };

    CommonKeyboardInteraction.prototype.closeDialog = function (target, e) {
      if (target.querySelector('.' + CLOSE_ICON)) {
        target.querySelector('.' + CLOSE_ICON).click();
        e.preventDefault();
        return;
      } else if (target.querySelector('.' + CANCEL_BTN_CLASS)) {
        target.querySelector('.' + CANCEL_BTN_CLASS).click();
        e.preventDefault();
        return;
      }
    };
  ...
  function () {
    ...
    ActionBase.prototype.getAggregateIcoPosition = function (id) {
      var groupingBarLayout = this.parent instanceof SfPivotView ? this.parent.parentElement.querySelector('.' + CHART_GROUPING_BAR_CLASS) : undefined;
      var pivotButton = (groupingBarLayout && this.parent.currentView === 'Chart' ? groupingBarLayout : this.parent.parentElement).querySelector('.' + PIVOT_BUTTON_CLASS + '.' + id);

      if (pivotButton && pivotButton.querySelector('.' + AXISFIELD_ICON_CLASS)) {
        var domInfo = pivotButton.querySelector('.' + AXISFIELD_ICON_CLASS).getBoundingClientRect();
        var cellInfo = {};
        cellInfo['top'] = domInfo.top;
        cellInfo['left'] = domInfo.left;
        cellInfo['bottom'] = domInfo.bottom;
        cellInfo['right'] = domInfo.right;
  ...
  function () {
    ...
    KeyboardInteraction.prototype.keyActionHandler = function (e) {
      ...
      if (target && sf.base.closest(target, '.' + PIVOT_FIELD_LIST_CONTAINER_CLASS)) {
        return;
      }
    ...
    KeyboardInteraction.prototype.toggleFieldList = function (e) {
      ...
      if (pivot && pivot.element && pivot.options.showFieldList && pivot.pivotFieldListModule) {
        pivot.pivotFieldListModule = pivot.pivotFieldListModule && pivot.pivotFieldListModule.blazor__instance ? pivot.pivotFieldListModule.blazor__instance : pivot.pivotFieldListModule;
        var fieldListIcon = pivot.element.querySelector('#' + pivot.element.id + 'fieldlist');

        if (pivot.options.showToolbar && pivot.toolbarModule && fieldListIcon && fieldListIcon.querySelector('button') && pivot.pivotFieldListModule.parentElement.classList.contains(POPUP_CLOSE)) {
          fieldListIcon.click();
          e.preventDefault();
          return;
        } else {
          if (pivot.element.querySelector('.' + TOGGLE_FIELD_LIST_CLASS)) {
            if (!pivot.element.querySelector('.' + TOGGLE_FIELD_LIST_CLASS).classList.contains(ICON_HIDDEN)) {
              ...
              e.preventDefault();
              return;
            } else if ((pivot.element.querySelector('.' + TOGGLE_FIELD_LIST_CLASS).classList.contains(ICON_HIDDEN) || pivot.options.showToolbar) && target && sf.base.closest(target, '.' + PIVOT_VIEW_CLASS) && pivot.pivotFieldListModule.parentElement && pivot.pivotFieldListModule.parentElement.classList.contains(POPUP_OPEN)) {
              ...
              if (dialogInstance && !dialogInstance.closeOnEscape) {
                ...
                pivot.closeDialog(pivot.pivotFieldListModule.parentElement, e);
              }
            }
          }
        }
  ...
  function () {
    ...
    Selection.prototype.clearSelection = function (targetElement, e, colIndex, rowIndex) {
      ...
      if (!e.shiftKey && !e.ctrlKey || this.parent.gridSettings.selectionSettings.type === 'Single') {
        ...
        } else if (this.parent.gridSettings.selectionSettings.mode === 'Both' || this.parent.gridSettings.selectionSettings.mode === 'Row') {
          if (targetElement.classList.contains(ROW_CELL_CLASS)) {
            for (var _b = 0, _c = [].slice.call(this.parent.element.querySelectorAll('.' + SELECTED_BGCOLOR + ', .' + CELL_SELECTED_BGCOLOR)); _b < _c.length; _b++) {
              var ele = _c[_b];
              sf.base.removeClass([ele], [CELL_ACTIVE_BGCOLOR, SELECTED_BGCOLOR, CELL_SELECTED_BGCOLOR]);
            }
          } else {
    ...
    DrillThrough.prototype.processClose = function (e) {
      ...
      if (target && sf.base.closest(target, '.' + POPUP + '.' + POPUP_OPEN)) {
        ...
        if (dialogInstance && !dialogInstance.closeOnEscape) {
          ...
          pivot_1.parent.closeDialog(sf.base.closest(target, '.' + POPUP + '.' + POPUP_OPEN), e);
        }
      }
    };
  ...
  function () {
    ...
    SfPivotView.prototype.getChartHeight = function (height) {
      ...
      if (this.options.showToolbar && this.options.showGroupingBar) {
        calculatedHeight = (height - (this.element.querySelector('.e-pivot-toolbar') ? this.element.querySelector('.e-pivot-toolbar').clientHeight : 42) - (this.element.querySelector('.' + CHART_GROUPING_BAR_CLASS) ? this.element.querySelector('.' + CHART_GROUPING_BAR_CLASS).clientHeight : 76)).toString();
        ...
        calculatedHeight = (height - (this.element.querySelector('.' + CHART_GROUPING_BAR_CLASS) ? this.element.querySelector('.' + CHART_GROUPING_BAR_CLASS).clientHeight : 76)).toString();
      }
    ...
    SfPivotView.prototype.contentReady = function () {
      ...
      if (this.options.renderGrid) {
        ...
        } else if (this.element && this.element.querySelectorAll('.e-axis-container').length > 0) {
          for (var _i = 0, _a = [].slice.call(this.element.querySelectorAll('.e-axis-container')); _i < _a.length; _i++) {
            var element = _a[_i];

            if (!sf.base.closest(element, '.' + PIVOT_FIELD_LIST_CONTAINER_CLASS)) {
    ...
    SfPivotView.prototype.createSheet = function (format, id, index) {
      ...
      var formatClass = '.format' + id + index;
      sheet.insertRule(formatClass + ', ' + formatClass + ' .e-cellvalue' + '{' + format + '}', 0);
    };
    ...
    SfPivotView.prototype.getSelectedCells = function () {
      ...
      for (var _i = 0, selectedElements_1 = selectedElements; _i < selectedElements_1.length; _i++) {
        ...
        if (!sf.base.isNullOrUndefined(colIndex) && !sf.base.isNullOrUndefined(rowIndex)) {
    ...
    SfPivotView.prototype.onContextMenuOpen = function () {
      if (this.options.allowGrouping && this.lastCellClicked) {
        ...
        if (currentCell && !currentCell.classList.contains(VALUESHEADER)) {
          var colIndex = Number(currentCell.getAttribute('aria-colindex'));
          var rowIndex = Number(currentCell.getAttribute('index'));
          var cellInfo = {};
    ...
    SfPivotView.prototype.updateGridUI = function (element, options, columnCount, rowCount) {
      ...
      if (content && !(content.scrollHeight > content.clientHeight)) {
        ...
        if (element.querySelector('.' + GRID_HEADER)) {
        ...
        if (element.querySelector('.' + HEADERCONTENT)) {
    ...
    SfPivotView.prototype.updateGridSettings = function (element, gridSetting, isSelectionUpdated) {
      if (element && gridSetting !== null) {
        this.gridSettings = JSON.parse(gridSetting);
      }

      if (isSelectionUpdated && this.selectionModule) {
    ...
    SfPivotView.prototype.closeDialog = function (target, e) {
      if (target.querySelector('.' + CLOSE_ICON)) {
        target.querySelector('.' + CLOSE_ICON).click();
        e.preventDefault();
        return;
  ...
  var PivotView = {
    ...
    updateGridSettings: function updateGridSettings(element, gridSetting, isSelectionUpdated) {
      if (element && element.blazor__instance) {
        element.blazor__instance.updateGridSettings(element, gridSetting, isSelectionUpdated);
      }
    },
    ...
    getAggregateIcoPosition: function getAggregateIcoPosition(element, id) {
      if (element && element.blazor__instance) {
        return element.blazor__instance.commonActionModule.getAggregateIcoPosition(id);
      }
